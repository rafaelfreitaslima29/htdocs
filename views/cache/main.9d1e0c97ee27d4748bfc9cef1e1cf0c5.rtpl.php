<?php if(!class_exists('Rain\Tpl')){exit;}?><meta charset="UTF-8">
	<!-- main início -->
    <main role="main" class="container-fluid">
    	<!-- conteiner início -->
    	<div class="conteiner">      		
      		<!-- row inicio -->
      		<div class="row">     	
				<!-- Título da página início -->
				<div class="col-md-12 mx-auto">     	
					<h2 class="text-center mb-5">Inicial</h2>
	      	    <!-- Título da página fim -->  
	      		</div>
				<!-- coluna 1 da esquerda início -->
	      		<div class="col-md-6 mb-5">
	      			<a class="btn btn-primary btn-lg btn-block" href="/?app=venda" role="button">REALIZAR VENDA</a>
	      			<a class="btn btn-success btn-lg btn-block" href="/?app=recebimento" role="button">REALIZAR RECEBIMENTO</a>
	      			<a class="btn btn-info btn-lg btn-block" href="/?app=relatorio-saldo-clientes" role="button">SALDO DO CLIENTE</a>
	      			<a class="btn btn-warning btn-lg btn-block" href="/?app=cadastrar_cliente" role="button">CADASTRAR NOVO CLIENTE</a>
	      		<!-- coluna 1 da esquerda fim -->
	      		</div>	      		
	      		<!-- coluna 2 da direita início -->
	      		<div class="col-md-6">
					<!-- Grupo de cartões inicio -->
					<div class="card-group">
					  <div class="card">
					    <img class="card-img-top" src="/views/res/images/como-ganhar-dinheiro-de-vendas.png" alt="Card image cap">
					    <div class="card-body">
					      <h5 class="card-title">VENDAS</h5>
					      <p class="card-text">Valor realizados com vendas hoje:<br><strong>R$25,55[falta implementar]</strong></p>				      
					    </div>
					  </div>
					  <div class="card">
					    <img class="card-img-top" src="/views/res/images/recebimentos.jpg" alt="Card image cap">
					    <div class="card-body">
					      <h5 class="card-title">RECEBIMENTOS</h5>
					      <p class="card-text">Valor recebidos hoje:<br><strong>R$33,75[falta implementar]</strong></p>
					    </div>
					  </div>
					<!-- Grupo de cartões fim -->  
					</div>
	      		<!-- coluna 2 da direita fim -->
	      		</div>       	
      	    <!-- row fim -->  
      		</div>
      <!-- conteiner fim -->	    
      </div>       
    <!-- main fim -->
    </main>   
  